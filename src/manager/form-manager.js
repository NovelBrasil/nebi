...
module.exports = class FormManager {
	...
	async loadAsync() {
		...
		this.tag_open_id = this.forum_channel.availableTags.find(
			(r) => r.name === `Aberto`,
		).id;
		this.tag_tutorando_plus_id = this.forum_channel.availableTags.find(
			(r) => r.name === `Tutorando+`,
		).id;
	}
	...
	add(userId, data) {
		if (this.#FORM.has(userId))
			...
			this.#FORM.set(
				userId,
				new Collection([[`${data.id}|${data.category}`, data.response]]),
			);
	}
	...
	async send(interaction) {
		...
		const user = await this.guild.members.fetch({
		...
		const tutorando_plus =
			this.#FORM.get(userId).get(`tutorando|data`) === `yes`;
		if (tutorando_plus) default_tag.push(this.tag_tutorando_plus_id);
		...
		for (const [key, value] of this.#FORM.get(userId)) {
			...
			const response =
				data.type === `select`
					? data.options.find((f) => f.value === value)
					: value;
			const embed = new EmbedBuilder()
				.setColor(purple_color)
				.setTitle(`${question}`)
				.setDescription(`> ${response}`);
			if (essential_ids.includes(id)) essential_embeds.push(embed);
	...
	async start(interaction) {
		...
		for (const [category, id_map] of Object.entries(config)) {
			const title =
				category === `data`
					? `Dados do Tutorando`
					: `Perguntas para avaliação de conhecimento`;
			const embed = new EmbedBuilder()
				.setColor(client.config.getColor(`form`))
				.setTitle(title);
			await interaction.user.send({ embeds: [embed] });
			for (const id of Object.keys(id_map)) {
				const response = await this.#sendQuestion(interaction, {
					category,
					id,
					text: title,
				});
	...
	async #sendQuestion(interaction, { category, id, text }) {
		...
		if (!data)
			throw Error(
				`Algo está errado na configuração da matrícula. Reporte isso para um membro da equipe de TI. Sentimentos muito pelo transtorno.`,
			);
		...
		const filter = !customId
			...
			: (interaction) =>
					(!button
						...
						: interaction.customId.split(`-`)[0]) === customId;
	...
	#getResponseById(userId, id) {
		if (!this.#FORM.has(userId))
			throw new Error(
				`O usuário nem iniciou a matrícula ou as respostas se perderam na cache.`,
			);
