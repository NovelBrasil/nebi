...
module.exports = {
	...
	run: async (client: Client, interaction: ButtonInteraction) => {
		...
		const title_input = new TextInputBuilder()
			...
			.setPlaceholder(
				`Exemplos: Encontrei um problema, desejo fazer um pagamento, etc.`,
			)
		...
		const modal_interaction = await interaction.awaitModalSubmit({
			filter,
			time: 600_000,
			errors: [`time`],
		});
		...
		embed.setDescription(
			`Você abriu um ticket. Por meio desse tópico você poderá conversar com um membro da equipe para te ajudar com quaisquer dúvidas ou problemas. Esse tópico será arquivado após **uma semana de inatividade**.`,
		);
		embed.setAuthor({
