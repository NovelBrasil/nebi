...
module.exports = {
    data: new SlashCommandBuilder()
        .setName(`set`)
        .setDescription(`Armazenar informações cruciais para o funcionamento do bot.`)
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addRoleOption((option) =>
                    option.setName(`valor`).setDescription(`A informação, no caso, o cargo que deseja armazenar.`)
                        .setRequired(true)
                )
        )
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addChannelOption((option) =>
                    option.setName(`valor`).setDescription(`A informação, no caso, o canal que deseja armazenar.`)
                        .setRequired(true)
                )
        )
    ,
    ...
    run: async (client, interaction) => {
        ...
        const value = subcommand.name == `canal` ? optionGet.channel.id : optionGet.role.id
        const data = await getData(key, token)
        if (data) await updateData({ key, value }, token)
        else await addData({ key, value }, token)
