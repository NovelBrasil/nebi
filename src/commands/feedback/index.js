...
async function getForumChannel(client, interaction) {
    const forumChannelID = await getData(`forum_feedback`, client.token, client)
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('formato')
                ...
                .addStringOption(option => // Using StringOption with autocomplete for multiple tags
                    option.setName('tags') // Renamed to 'tags' to indicate multiple
                        .setDescription('As tags para o tópico (selecione múltiplos separando por vírgula).')
                        .setRequired(true) // At least one tag is required
                        .setAutocomplete(true)) // Enable autocomplete for dynamic tag suggestions
                .addBooleanOption(option => // Added boolean option for Fireplume
            option.setName('fireplume')
    ...
    async execute(client, interaction) {
        ...
        const tagId = interaction.options.getString('tags');
        const hasFireplumeText = interaction.options.getBoolean('fireplume'); // Get the boolean value
        ...
        const tagEmAndamento =  this.forum_channel.availableTags.find(
          (r) => r.name === `Em andamento`,
        ).id;

        const tagFireplume =  this.forum_channel.availableTags.find(
          (r) => r.name === `Fireplume`,
        ).id;
        ...
        try {
            ...
            const newThread = await forumChannel.threads.create({
                ...
                appliedTags: [tagId, tagEmAndamento].concat(hasFireplumeText ? [tagFireplume] : []), // Apply the selected tag ID
            });
            ...
            await interaction.editReply({
                content: 'Ocorreu um erro ao criar o tópico. Por favor, verifique minhas permissões e tente novamente.',
            });
        }
    },
    async autocomplete(client, interaction) {
        ...
        const filteredTags = availableTags.filter(tag => !excludedTagNames.includes(tag.name));

        // Filter tags based on the user's input
        const choices = filteredTags
            .filter(tag => tag.name.toLowerCase().includes(focusedValue.toLowerCase()))
