...
function getYourBadgesPage(client, badges) {
    console.log(client.config.emoji.getEmoji(`developer`))
    const activies = badges.filter(b => b.enabled).map(b => client.config.emoji.getEmoji(b.name))
    const disabled = badges.filter(b => !b.enabled).map(b => client.config.emoji.getEmoji(`disable_${b.name}`))
    ...
    const embed = new EmbedBuilder({
        ...
        fields: badges.length > 0 ? [
            {
                name: `Habilitadas`,
                value: activies.length > 0 ? activies.join(` `) : `**Você está com todas medalhas desabilitadas.**`,
                inline: false
            },
            {
                name: `Desabilitadas`,
                value: disabled.length > 0 ? disabled.join(` `) : `**Você está com todas medalhas habilitadas.**`,
                inline: false
            }
        ] : [],
        author: {
