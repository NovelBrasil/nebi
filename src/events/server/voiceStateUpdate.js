...
function handleLeftVC(client, user, guildId, channelId) {
	Log(client, user, "Usuário saiu de canal de voz", `Um usuário saiu do canal de voz https://discord.com/channels/${guildId}/${channelId}`)
}

function handleChangeVc(client, user, oldGuildId, oldChannelId, newGuildId, newChannelId) {
	Log(client, user, "Usuário mudou de canal de voz", `Um usuário mudou do canal de voz https://discord.com/channels/${oldGuildId}/${oldChannelId} para o canal de voz https://discord.com/channels/${newGuildId}/${newChannelId}`)
}
...
function handleSelfMuted(client, user) {
	Log(client, user, "Usuário se silenciou", "Um usuário silenciou a si mesmo em canal de voz.")
}

function handleSelfUnmuted(client, user) {
	Log(client, user, "Usuário se dessilenciou", "Um usuário dessilenciou a si mesmo em canal de voz.")
}

function handleSelfDeafen(client, user) {
	Log(client, user, "Usuário desativou o áudio", "Um usuário desativou o próprio áudio em canal de voz.")
}

function handleSelfUndeafen(client, user) {
	Log(client, user, "Usuário reativou o áudio", "Um usuário resativou o próprio áudio em canal de voz.")
}

function handleCameraOn(client, user) {
	Log(client, user, "Usuário ligou a câmera", "Um usuário ligou a câmera em canal de voz.")
}

function handleCameraOff(client, user) {
	Log(client, user, "Usuário desligou a câmera", "Um usuário desligou a câmera em canal de voz.")
}

function handleScreenSharingStart(client, user) {
	Log(client, user, "Usuário começou a compartilhar a tela", "Um usuário começou a compartilhar a tela em canal de voz.")
}

function handleScreenSharingEnd(client, user) {
	Log(client, user, "Usuário parou de compartilhar a tela", "Um usuário parou de compartilhar a tela em canal de voz.")
}

function handleServerMuted(client, user) {
	Log(client, user, "Usuário foi silenciado", "Um usuário foi silenciado administrativamente em canal de voz.")
}

function handleServerUnmuted(client, user) {
	Log(client, user, "Usuário foi dessilenciado", "Um usuário foi dessilenciado administrativamente em canal de voz.")
}

function handleServerDeafen(client, user) {
	Log(client, user, "Usuário teve o áudio desativado", "Um usuário teve o áudio desativado administrativamente em canal de voz.")
}

function handleServerUndeafen(client, user) {
	Log(client, user, "Usuário teve o áudio reativado", "Um usuário teve o áudio reativado administrativamente em canal de voz.")
}
...
module.exports = async (client, oldState, newState) => {
	...
	try {
		...
		if (changedVC) handleChangeVc(client, user, oldState.guild.id, oldChannel, newState.guild.id, newChannel);
